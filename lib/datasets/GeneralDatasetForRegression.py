...
class GeneralDatasetForRegression(data.Dataset):

  def __init__(self, transform, data_indicator):
  ...
  def __repr__(self):
    return ('{name}(point-num={NUM_PTS}, length={length}, dataset={dataset_name})'.format(name=self.__class__.__name__, **self.__dict__))
  ...
  def _process_(self, image, target, index):
    ...
    return image, mask, points, torch_index, torch_nopoints, ori_size
